...
describe('Floating-point', function () {
  ...
  var examples = [
    ...
    { data: Infinity, encoded: Buffer.from('f97c00', 'hex'), symmetric: true },
    { data: NaN, encoded: Buffer.from('f97e00', 'hex'), symmetric: true },
    { data: -Infinity, encoded: Buffer.from('f9fc00', 'hex'), symmetric: true },
  ...
  examples.forEach(function (example, index) {
    ...
    if (example.symmetric) {
      it('Example ' + index + " (encode)", function () {
        ...
        assert.deepEqual(Buffer.from(encoded), example.encoded)
      })
    }
  })
})
